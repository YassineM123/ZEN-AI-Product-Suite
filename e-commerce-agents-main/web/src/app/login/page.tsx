...
export default function LoginPage() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    setIsLoading(true);
    try {
      await login(email || "executive@zen-groupe.fr", password || "demo_pass");
  ...
  const handleDemoAccess = async () => {
    setIsLoading(true);
    await login("executive@zen-groupe.fr", "demo");
    router.push("/operations");
  };
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="rounded-xl border border-purple-500/30 bg-purple-950/20 p-4 text-center space-y-3">
          ...
          <p className="text-xs text-slate-400">
            Access the live AI Operations Center with pre-loaded enterprise telemetry and stockout simulations.
          </p>
          <Button
        ...
        <Card className="border-slate-800 bg-slate-900/80 backdrop-blur">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-semibold text-white">Sign in to Enterprise Account</CardTitle>
            <CardDescription className="text-xs text-slate-400">
              Enter your credentials to access the commerce operations dashboard.
            </CardDescription>
          </CardHeader>
          <CardContent>
